...
final class UserNotificationService: NotificationService, @unchecked Sendable {
    ...
    func requestAuthorization() async throws -> Bool {
        guard let center else {
            Self.log.warning("Notification authorization unavailable — not a bundled app")
            return false
        }
        let options: UNAuthorizationOptions = [.alert, .badge, .sound]
        let granted = try await center.requestAuthorization(options: options)
        Self.log.info("Notification authorization result=\(granted)")
        return granted
    }
    ...
    private func send(title: String, body: String, task: TaskRecord) async {
        ...
        do {
            ...
            Self.log.info("Notification posted title=\(title, privacy: .public) taskId=\(task.taskId, privacy: .public)")
        } catch {
            Self.log.warning("Notification failed title=\(title, privacy: .public) error=\(error.localizedDescription, privacy: .public)")
        }
    }
    ...
    private func sendExternal(title: String, body: String, snapshot: ExternalTaskSnapshot) async {
        ...
        do {
            try await center.add(request)
            Self.log.info("External notification posted title=\(title, privacy: .public) snapshotId=\(snapshot.id, privacy: .public)")
        } catch {
            Self.log.warning("External notification failed title=\(title, privacy: .public) error=\(error.localizedDescription, privacy: .public)")
        }
    }
}
